...
int query(int node,int a,int b,vector<int> &M,int i,int j){
    ...
    int mid = (b-a)>>1;
    int p1=query(node<<1,a,a+mid,M,i,j);
    int p2=query((node<<1)+1,a+mid+1,b,M,i,j);

    return max(p1,p2);
}

void update(int node, int a, int b, vector<int> &M,int i,int j,int value) {
    ...
    int mid = (b-a)>>1;
	update(node<<1, a, a+mid,M,i,j,value);
	update((node<<1)+1, a+mid+1, b, M,i,j,value); 
